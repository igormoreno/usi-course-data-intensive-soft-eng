...
public class SupplierServiceImpl extends SupplierService {
	...
	public Dataset<Supplier> getSupplierRefListInSupplies(conditions.Condition<conditions.ProductAttribute> suppliedProduct_condition,conditions.Condition<conditions.SupplierAttribute> supplierRef_condition)		{
		...
		// For role 'supplierRef' in reference 'prodSupplied'. A->B Scenario
		...
		if(suppliedProduct_refilter.booleanValue()) {
			if(all == null)
				all = new ProductServiceImpl().getProductList(suppliedProduct_condition);
			joinCondition = null;
			joinCondition = productTDOprodSuppliedsuppliedProduct.col("productID").equalTo(all.col("productID"));
			if(joinCondition == null)
				productTDOprodSuppliedsuppliedProduct = productTDOprodSuppliedsuppliedProduct.as("A").join(all).select("A.*").as(Encoders.bean(ProductTDO.class));
			else
				productTDOprodSuppliedsuppliedProduct = productTDOprodSuppliedsuppliedProduct.as("A").join(all, joinCondition).select("A.*").as(Encoders.bean(ProductTDO.class));
		}
	
		
		Dataset<Row> res_prodSupplied = supplierTDOprodSuppliedsupplierRef.join(productTDOprodSuppliedsuppliedProduct
				.withColumnRenamed("productID", "Product_productID")
				.withColumnRenamed("unitsInStock", "Product_unitsInStock")
				.withColumnRenamed("unitsOnOrder", "Product_unitsOnOrder")
				.withColumnRenamed("productName", "Product_productName")
		...
		Dataset<Supplier> res_Supplier_prodSupplied = res_prodSupplied.select( "supplierID", "address", "city", "companyName", "contactName", "contactTitle", "country", "fax", "homePage", "phone", "postalCode", "region", "logEvents").as(Encoders.bean(Supplier.class));
		
		res_Supplier_prodSupplied = res_Supplier_prodSupplied.dropDuplicates(new String[] {"supplierID"});
		datasetsPOJO.add(res_Supplier_prodSupplied);
		
		
		Dataset<Supplies> res_supplies_supplierRef;
		...
		if(supplierRef_refilter.booleanValue())
			res = res.filter((FilterFunction<Supplier>) r -> supplierRef_condition == null || supplierRef_condition.evaluate(r));
	...
	public boolean insertSupplierInSuppliersFromMyMongoDB(Supplier supplier)	{
		String idvalue="";
		idvalue+=supplier.getSupplierID();
		...
		docSuppliers_1.append("SupplierID",supplier.getSupplierID());
		...
		filter = eq("SupplierID",supplier.getSupplierID());
	...
	public void updateSupplierListInSuppliersFromMyMongoDB(Condition<SupplierAttribute> condition, SetClause<SupplierAttribute> set) {
		...
		if(!refilter.booleanValue()) {
			...
			if(!inUpdateMethod || allSupplierIdList == null)
				allSupplierIdList = this.getSupplierList(condition).select("supplierID").collectAsList();
			...
			for(Row row : allSupplierIdList) {
				...
				conditionId = Condition.simple(SupplierAttribute.supplierID, Operator.EQUALS, row.getAs("supplierID"));
	...
	public void updateSupplierRefListInSupplies(
	...
	public void deleteSupplierRefListInSupplies(	
		conditions.Condition<conditions.ProductAttribute> suppliedProduct_condition,	
		conditions.Condition<conditions.SupplierAttribute> supplierRef_condition){
			//TODO
		}
