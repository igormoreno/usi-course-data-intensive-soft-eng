...
public abstract class CustomerService {
	...
	static {
		defaultLoadingParameters.put(ROLE_NAME.BUYS_CUSTOMERREF, loading.Loading.LAZY);
	}
	...
	public Customer getCustomer(Customer.buys role, Order order) {
		if(role != null) {
			if(role.equals(Customer.buys.customerRef))
				return getCustomerRefInBuysByBoughtOrder(order);
		}
		return null;
	}
	...
	public Dataset<Customer> getCustomerList(Customer.buys role, Condition<OrderAttribute> condition) {
		if(role != null) {
			if(role.equals(Customer.buys.customerRef))
				return getCustomerRefListInBuysByBoughtOrderCondition(condition);
		}
		return null;
	}
	...
	public Dataset<Customer> getCustomerList(Customer.buys role, Condition<OrderAttribute> condition1, Condition<CustomerAttribute> condition2) {
		if(role != null) {
			if(role.equals(Customer.buys.customerRef))
				return getCustomerRefListInBuys(condition1, condition2);
		}
		return null;
	}
	...
	public abstract Dataset<Customer> getCustomerRefListInBuys(conditions.Condition<conditions.OrderAttribute> boughtOrder_condition,conditions.Condition<conditions.CustomerAttribute> customerRef_condition);
	
	public Dataset<Customer> getCustomerRefListInBuysByBoughtOrderCondition(conditions.Condition<conditions.OrderAttribute> boughtOrder_condition){
		return getCustomerRefListInBuys(boughtOrder_condition, null);
	}
	
	public Customer getCustomerRefInBuysByBoughtOrder(pojo.Order boughtOrder){
		if(boughtOrder == null)
			return null;
	
		Condition c;
		c=Condition.simple(OrderAttribute.orderID,Operator.EQUALS, boughtOrder.getOrderID());
		Dataset<Customer> res = getCustomerRefListInBuysByBoughtOrderCondition(c);
		return !res.isEmpty()?res.first():null;
	}
	
	public Dataset<Customer> getCustomerRefListInBuysByCustomerRefCondition(conditions.Condition<conditions.CustomerAttribute> customerRef_condition){
		return getCustomerRefListInBuys(null, customerRef_condition);
	}
	...
	public abstract void updateCustomerRefListInBuys(
	...
	public abstract void deleteCustomerRefListInBuys(	
		conditions.Condition<conditions.OrderAttribute> boughtOrder_condition,	
		conditions.Condition<conditions.CustomerAttribute> customerRef_condition);
