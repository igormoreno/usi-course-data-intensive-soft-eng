...
public class SupplierServiceImpl extends SupplierService {
	...
	public Dataset<Supplier> getSupplierRefListInSupplies(conditions.Condition<conditions.ProductAttribute> suppliedProduct_condition,conditions.Condition<conditions.SupplierAttribute> supplierRef_condition)		{
		...
		// For role 'suppliedProduct' in reference 'supplierRef'  B->A Scenario
		...
		if(suppliedProduct_refilter.booleanValue()) {
			...
			joinCondition = null;
			joinCondition = productTDOsupplierRefsuppliedProduct.col("productID").equalTo(all.col("productID"));
			if(joinCondition == null)
				productTDOsupplierRefsuppliedProduct = productTDOsupplierRefsuppliedProduct.as("A").join(all).select("A.*").as(Encoders.bean(ProductTDO.class));
			else
				productTDOsupplierRefsuppliedProduct = productTDOsupplierRefsuppliedProduct.as("A").join(all, joinCondition).select("A.*").as(Encoders.bean(ProductTDO.class));
		}
		Dataset<Row> res_supplierRef = 
			supplierTDOsupplierRefsupplierRef.join(productTDOsupplierRefsuppliedProduct
